...
def find_event_by_instance(log_data: dict, instance_id: str, event_type: str):
    ...
    for _, org_data in organisations.items():
        ...
        for event in events:
            event_party_id, event_instance_id = event.get("instanceId").split('/')
...
def _write_json_file(log_data: Dict[str, Any], file_path: str) -> None:
    ...
    # Convert to lakehouse path format
    if not file_path.startswith("file:/lakehouse/default/"):
        lakehouse_path = f"file:/lakehouse/default/Files/{file_path}"
    else:
        lakehouse_path = file_path
    
    # 1. Load existing data and create backup
    try:
        existing_content = mssparkutils.fs.head(lakehouse_path)
        existing_log_data = json.loads(existing_content)
        
        # Create backup
        backup_path = lakehouse_path.replace('.json', '_backup_log.json')
        mssparkutils.fs.put(backup_path, existing_content, True)
        
        # Use existing data as base
        merged_log_data = existing_log_data.copy()
        
    except Exception:
        # File doesn't exist, use provided log_data as base
        merged_log_data = log_data.copy()
    
    # 3. Write merged data
    json_content = json.dumps(log_data, ensure_ascii=False, indent=2)
    mssparkutils.fs.put(lakehouse_path, json_content, True)
...
class InstanceTracker:
    ...
    def from_log_file(cls, path_to_json_file: str):
        try:
            data = json.loads(mssparkutils.fs.head(path_to_json_file))
    ...
    def has_processed_instance(self, org_number: str, digitaliseringstiltak_report_id: str, event_type: str) -> bool:
    ...
    def logging_instance(self, org_number: str, digitaliseringstiltak_report_id: str, instance_meta_data: dict, event_type: str):
        ...
        instance_log_entry = {
            ...
            "processed_timestamp": dt.datetime.utcnow().isoformat(),
