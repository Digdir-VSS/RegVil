    ...
    try:
        ...
        if event.get("type") == "app.instance.process.completed":
            ...
            if not download_params:
                logging.error(
                    f"Download failed for app name: {app_name} party id: {party_id} instance id: {instance_id}."
                )
                return (
                    f"Download failed for app name: {app_name} party id: {party_id} instance id: {instance_id},",
                    download_response,
            ...
            if app_name == "regvil-2025-slutt":
                logging.info(
                    f"Terminal app reached: {app_name}. No further processing."
                )
            ...
            if result == 200:
                ...
                if notification_results == 200:
                    logging.info(
                        f"Notification sent successfully for app name: {app_name} party id: {party_id} instance id: {instance_id}."
                    )
                    print(
                        f"Notification sent successfully for app name: {app_name} party id: {party_id} instance id: {instance_id}."
                    )
                    ...
                    logging.error(
                        f"Notification failed for app name: {app_name} party id: {party_id} instance id: {instance_id}. Status code: {notification_results}"
                    )
                    print(
                        f"Notification failed for app name: {app_name} party id: {party_id} instance id: {instance_id}. Status code: {notification_results}"
                    )
